...
# Monolingual Corpora
src_corpus=$PWD/XLM/data/wiki/txt/$1.all
tgt_corpus=$PWD/XLM/data/wiki/txt/$2.all
concat_corpus=$PWD/concat/$1-$2.wikipedia.tok
...
JOINT_EMBED=$LANG_PAIR_PATH/fasttext.${src_lang}-${tgt_lang}.word.joint.300.vec
...
# mkdir $PWD/word_embeddings
mkdir $LANG_PAIR_PATH
#mkdir $PWD/concat
#down (or up) sample a side of the corpus and concatenate them
a=($(wc -l $tgt_corpus))
python sample_corpus.py  --corpus $src_corpus --target_size ${a[0]} --output $src_corpus.sampled
cat $src_corpus.sampled $tgt_corpus | shuf > $concat_corpus

# train joint fastText embeddings
$FASTTEXT skipgram -dim 300 -thread 8 -input $concat_corpus -output $JOINT_EMBED
